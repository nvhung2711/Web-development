...
let arr_ = ["1", 2, "4", "591", 392, "391", 2, 5, "10", 2, "1", "1", 1, 20, 20];

arrSort(arr_);
// ["1", "4", "591", "391", "10", "1", "1", 2, 392, 2, 5, 2, 1, 20, 20]

function arrSort(argument) {
    switch (typeof arr_[0]) {
        case "string":
            let arr2 = arr_.filter(function(num, i) {
                return typeof arr_[i] == "string";
            });
            let arr3 = arr_.filter(function(num, i) {
                return typeof arr_[i] == "number";
            });
            arr_ = arr2.concat(arr3);
            break;
        case "number":
            let arr4 = arr_.filter(function(num, i) {
                return typeof arr_[i] == "number";
            });
            let arr5 = arr_.filter(function(num, i) {
                return typeof arr_[i] == "string";
            });
            arr_ = arr4.concat(arr5);
            break;
    }
    return arr_;
}